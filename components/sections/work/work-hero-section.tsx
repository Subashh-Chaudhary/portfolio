...
const GlitchText = ({ text }: { text: string }) => {
    return (
        <span className="relative inline-block group/glitch">
            ...
            <span className="absolute top-0 left-0 -z-10 text-purple-500 opacity-0 group-hover/glitch:opacity-70 group-hover/glitch:animate-glitch-1">
                {text}
            </span>
            <span className="absolute top-0 left-0 -z-10 text-pink-500 opacity-0 group-hover/glitch:opacity-70 group-hover/glitch:animate-glitch-2">
                {text}
            </span>
        </span>
    )
...
export function WorkHeroSection() {
    ...
    return (
        <motion.section
            ...
            initial="hidden"
            animate="visible"
            className="relative py-16 sm:py-20 md:py-24 lg:py-32 xl:py-40 bg-slate-50 overflow-hidden"
        >
            {/* Animated Gradient Background */}
            <div className="absolute inset-0 z-0 opacity-30">
                <div className="absolute inset-0 bg-gradient-to-br from-purple-500/10 via-pink-500/10 to-orange-500/10 animate-gradient-shift" />
            ...
            <div className="absolute inset-0 z-0 opacity-20 pointer-events-none">
                ...
                />
                <motion.div
                    className="absolute bottom-10 right-10 w-4 h-4 border-b border-r border-orange-400"
                    animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.6, 0.3] }}
            ...
            {/* HUD Elements */}
            <div className="absolute top-4 left-4 sm:top-6 sm:left-6 md:top-8 md:left-8 z-20 font-mono text-[10px] md:text-xs text-purple-400/60 hidden md:block">
                <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                    <span>MODULE: WORK_SHOWCASE</span>
                </div>
                <div>STATUS: ONLINE</div>
                <div>PROJECTS: {totalProjects}</div>
            </div>

            <div className="absolute top-4 right-4 sm:top-6 sm:right-6 md:top-8 md:right-8 z-20 font-mono text-[10px] md:text-xs text-pink-400/60 hidden md:block text-right">
                <div>COORD: {mousePosition.x.toFixed(2)}, {mousePosition.y.toFixed(2)}</div>
                <div>RENDER: PROJECT_ENGINE_V1</div>
            ...
            <div className="relative z-10 px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-20">
                <motion.div variants={staggerItem}>
                    ...
                    >
                        <motion.div
                            className="h-px w-12 sm:w-16 md:w-20 bg-gradient-to-r from-purple-500 to-pink-500"
                    ...
                    >
                        <span className="text-gray-400 bg-clip-text bg-gradient-to-r from-white via-purple-200 to-gray-500">
                            <GlitchText text="MY" />
                        </span>
                        <br />
                    ...
                    >
                        ...
                        <p className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-300 font-light pl-4 sm:pl-6 md:pl-8 leading-relaxed max-w-3xl">
                            A curated collection of my best work, showcasing full-stack development, modern web technologies, and creative problem-solving.
                        </p>
                    </motion.div>
                    ...
                    >
                        {[
                            { icon: Code2, label: 'Total Projects', value: totalProjects, gradient: 'from-purple-500 to-pink-500', suffix: '' },
                            { icon: Star, label: 'Featured', value: featuredCount, gradient: 'from-pink-500 to-orange-500', suffix: '' },
                            { icon: Sparkles, label: 'Technologies', value: techCount, gradient: 'from-orange-500 to-red-500', suffix: '+' }
                        ].map((stat, idx) => (
                            <motion.div
                                key={stat.label}
                                ...
                                className="group relative bg-gradient-to-br from-white to-slate-100 border border-slate-200 hover:border-transparent transition-all duration-300 p-4 sm:p-5 md:p-6 overflow-hidden"
                            >
