...
class AnimationController {
    ...
    // Set up custom random number generator for determinism
    ...
    // Project 3D point onto 2D viewport
    ...
    // Destroy the animation timeline instance
    public destroy() {
        this.timeline.kill()
    }
}
...
class Star {
    ...
    private rotationDirection: number // -1 or 1
    private expansionRate: number // rate of expansion outwards
    ...
    constructor(cameraZ: number, cameraTravelDistance: number) {
        ...
        this.expansionRate = 1.2 + Math.random() * 0.8 // expansion multiplier (1.2 to 2.0)
        this.finalScale = 0.7 + Math.random() * 0.6 // final size ratio (0.7 to 1.3)
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            // Mixed easing functions: smooth start, elastic end
            ...
            // Phase-based motion paths
            if (displacementProgress < 0.3) {
                // Phase 1: straight path projection (30% length)
                ...
                // Phase 2: curved displacement (40% length)
                ...
                // Base coordinates at 30% mark
                const baseX = spiralPos.x + this.dx * 0.3;
                const baseY = spiralPos.y + this.dy * 0.3;

                // Target coordinates at 70% mark
                const targetX = spiralPos.x + this.dx * 0.7;
                const targetY = spiralPos.y + this.dy * 0.7;

                // Adding curve deviations
                ...
                // Phase 3: stronger spiral dispersion (last 30%)
                const finalProgress = (displacementProgress - 0.7) / 0.3;

                // Base coordinates at 70% mark
                const baseX = spiralPos.x + this.dx * 0.7;
                const baseY = spiralPos.y + this.dy * 0.7;

                // Expanded coordinates target
            ...
            // Map 2D screen coordinate values to 3D space vectors
            ...
            // Particle size scaling animation
            let sizeMultiplier = 1.0;
            if (displacementProgress < 0.6) {
                // First 60% transition: slight swelling
                sizeMultiplier = 1.0 + displacementProgress * 0.2;
            } else {
                // Remaining 40% transition: scaling to final size factor
...
export function SpiralAnimation({ loop = true }: SpiralAnimationProps = {}) {
    ...
    // Initialize and track animation frame controller
    useEffect(() => {
